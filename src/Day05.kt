fun main() {
    ...
    check(part1(testParsed.first, testParsed.second) == 143)
    check(part2(testParsed.first, testParsed.second) == 123)

    val input = readInputAsText("Day05")
    val parsed = parse(input)
    part1(parsed.first, parsed.second).println()
    part2(parsed.first, parsed.second).println()
}
...
private fun part1(inedges: Map<Int, List<Int>>, updates: List<List<Int>>) =
    updates.filter { topologicalSort(inedges, it) == it }.sumOf { it[it.size / 2] }

private fun part2(inedges: Map<Int, List<Int>>, updates: List<List<Int>>) =
